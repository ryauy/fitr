...
class OutfitService {
    ...
    func getOutfitRecommendation(userId: String, weather: WeatherData, clothingItems: [ClothingItem], completion: @escaping (Result<Outfit, Error>) -> Void) {
    ...
    private func getRecommendationFromBackend(userId: String, weather: WeatherData, clothingItems: [ClothingItem], completion: @escaping (Result<Outfit, Error>) -> Void) {
    ...
    private func getLocalRecommendation(userId: String, weather: WeatherData, clothingItems: [ClothingItem], completion: @escaping (Result<Outfit, Error>) -> Void) {
        ...
        let temperatureF = weather.main.temperatureF
        
        // Temperature-based tags
        if temperatureF > 77 { // > 25°C
            appropriateWeatherTags.append(.hot)
        } else if temperatureF > 64 { // > 18°C
            appropriateWeatherTags.append(.warm)
        } else if temperatureF > 50 { // > 10°C
        ...
        let weatherCondition = weather.weather.first?.main.lowercased() ?? ""
    ...
    private func createOutfitDescription(items: [ClothingItem], weather: WeatherData) -> String {
        let itemDescriptions = items.map { "\($0.color) \($0.type.rawValue.lowercased())" }
        let condition = weather.weather.first?.description.capitalized ?? "current"
        
        var description = "For \(condition) weather (\(weather.main.temperatureF)°F), "
